	   ...
	   while(iterarItems.hasNext()) {
		...
		if (mostrarHechizo) {
			...
			if("Hechizo 1".equals(tipoHechizoActual)){
				...
				colorHechizo= new Color(100, 100, 255, 0);
			}
			else if 
				("Hechizo 2".equals(tipoHechizoActual)){
		...
		//entorno.dibujarCirculo(hechizoX, hechizoY, 100, Color.WHITE);
		...
		if (! hizoClickEnBoton && hechizoSeleccionado != null) {
			...
			for (int i = activos.size() - 1; i >= 0; i--) {
				...
				if (distancia <= radio) {
					...
					else if (hechizoSeleccionado =="Hechizo 2" && mana >= 20) {
